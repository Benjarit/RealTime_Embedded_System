...
#define NUM_THREADS2 15
#define NUM_THREADS3 225
...
//---------- Data for one thread for entire matrix -------------//
	if( chosen == 1 ){
		...
		thread_data1[0].ending_element = 225;
	}
...
//---------- Data for one thread for each row -------------//
		...
		for(i = 0; i < NUM_THREADS1; i ++){
			...
			thread_data2[i].ending_element = temp+15;
			temp += 15;
		}
	}
...
//---------- Data for one thread for each column -------------//
		...
		for(i = 0; i < NUM_THREADS2; i++){
			...
			thread_data3[i].ending_element = i+(c1*14);
		 }
	}
