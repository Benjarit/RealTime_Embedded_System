...
//---------- Data for one thread for each row -------------//
		...
		thread_data2[0].starting_element = 0;
		thread_data2[0].ending_element = 100;
		thread_data2[1].starting_element = 100;
		thread_data2[1].ending_element = 200;
	}
...
//---------- Data for one thread for each column -------------//
		...
		for(i = 0; i < NUM_THREADS2; i++){
			...
			thread_data3[i].ending_element = i+c1;
		 }
	}
